...
function myListArticle(param, success, fail) {
  local.get(`${url}/myList`, { params: param }).then(success).catch(fail);
}

function getImg(saveFolder, originalName, saveFile, success, fail) {
  local.get(`${url}/getImg/${saveFolder}/${originalName}/${saveFile}`).then(success).catch(fail);
}
