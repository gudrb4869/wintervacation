...
import { registArticle, modifyArticle, detailArticle } from "@/api/qna-board";
...
const writeArticle = () => {
  console.log("qna게시글 등록!!!", article.value);
  registArticle(
    article.value,
    (response) => {
...
const updateArticle = () => {
  console.log(article.value.article_no + "번 qna게시글 수정!!!");
  modifyArticle(
    article.value,
    (response) => {
      console.log(response);
